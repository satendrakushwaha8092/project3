...
try {
    ...
    if (Object.entries(data).length == 0) {
        ...
        //     return res.status(400).send({status:false,message:'Title should be Mr ,Mrs.Miss,Mast'})
       ...
       let isValidPass= /^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/.test(password)
        if(!isValidPass){
